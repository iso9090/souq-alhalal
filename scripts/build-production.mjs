...
if(config.mode!=='production'||config.datasource!=='production'||config.authProvider!=='firebase'||config.writesEnabled!==false)throw Error('Production configuration incomplete or writes rollout not approved');
...
fs.writeFileSync(path.join(out,'publish-manifest.json'),JSON.stringify({mode:'production',datasource:'production',writesEnabled:false,legacy:'EXCLUDED',files:manifest.files},null,2));
