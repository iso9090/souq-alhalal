...
export async function createProductionDatasource({sdk,db,auth,countries={},config={}}={}){
 ...
 async function loadUser(uid){
  if(typeof uid!=='string'||!uid||uid.includes('/')||uid==='.'||uid==='..'||!allowed('users_view'))return null;
  const token=epoch,currentUid=actor().uid,valid=()=>!disposed&&token===epoch&&actor().uid===currentUid&&allowed('users_view');
  try{const snapshot=await sdk.getDoc(sdk.doc(db,'users',uid));if(!valid())return null;state.capabilities['user:'+uid]={status:snapshot.exists()?'available':'empty',bounded:true};return snapshot.exists()?normalizeProductionRecord(uid,snapshot.data(),'users'):null;}
  catch(error){if(valid())state.capabilities['user:'+uid]={status:String(error.code).includes('permission-denied')?'denied':'error',code:String(error.code||'READ_FAILED'),bounded:true};return null;}
 }
