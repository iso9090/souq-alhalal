...
(async () => {
 ...
 await page.addInitScript(() => {
   ...
   const mock = {
     ...
     runTransaction:async(db,fn)=>{
       const run=queue.then(async()=>{const b=batch();const result=await fn({get:async ref=>snapshot(ref),set:b.set,update:b.update});if(window.__mock.failTransaction)throw Error('test transaction failure');await b.commit();return result});
 ...
 await page.locator('.commercial-map article').first().waitFor();assert.equal(await page.locator('.commercial-map article').count(),10);pass('ad map ten placements');assert.equal(await page.locator('.commercial-hero-slots section').count(),10);pass('ten distinct Hero slots in map');
 await page.getByRole('button',{name:'إضافة إعلان جديد',exact:true}).click();await page.locator('dialog[open]').waitFor();assert.equal(await page.locator('[name=heroOrder] option').count(),10);pass('Hero order selector exposes ten occupancy labels');for(const [key,value]of Object.entries({title:'تعارض معزول',advertiserName:'معلن الاختبار',targetUrl:'https://example.test/',startAt:'2026-01-01T10:00',endAt:'2028-01-01T10:00'}))await page.locator('[name='+key+']').fill(value);await page.locator('dialog input[type=file]').setInputFiles(path.join(root,'hero-livestock.png'));await page.getByText('الصورة جاهزة للحفظ.',{exact:true}).waitFor();await page.locator('[name=heroOrder]').selectOption('1');await page.getByRole('button',{name:'حفظ الإعلان',exact:true}).click();await page.getByText('هذا الترتيب مستخدم خلال الفترة المحددة. اختر مكانًا متاحًا.',{exact:true}).waitFor();assert.equal(await page.evaluate(()=>[...window.__mock.docs.values()].some(a=>a.title==='تعارض معزول')),false);pass('occupied Hero slot blocked without write');await page.getByRole('button',{name:'إغلاق',exact:true}).click();
 ...
 await page.setViewportSize({width:1440,height:900});await page.getByRole('button',{name:'إضافة إعلان جديد',exact:true}).click();await page.locator('dialog[open]').waitFor();await page.locator('[name=placement]').selectOption('middle');
 await page.locator('dialog input[type=file]').setInputFiles(path.join(root,'hero-livestock.png'));await page.getByText('الصورة جاهزة للحفظ.',{exact:true}).waitFor();
 for(const [key,value] of Object.entries({title:'إعلان جديد معزول',advertiserName:'معلن خاص',targetUrl:'https://example.test/',startAt:'2026-01-01T10:00',endAt:'2028-01-01T10:00',priority:'4',advertiserEmail:'private@example.test',advertiserPhone:'0500000000'}))await page.locator('[name='+key+']').fill(value);
 ...
 for(const width of [1440,768,390,360]){await page.setViewportSize({width,height:1000});await page.locator('dialog').evaluate(d=>{d.scrollTop=0;});await page.locator('.choose-image').scrollIntoViewIfNeeded();assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false);await page.locator('dialog').screenshot({path:path.join(shots,'device-image-'+width+'.png')});pass('device image full-app dialog '+width);}
 await page.setViewportSize({width:1440,height:1000});await page.evaluate(()=>window.__mock.failTransaction=true);await page.getByRole('button',{name:'حفظ الإعلان',exact:true}).click();await page.getByText('تعذر الحفظ. لم يتم تأكيد العملية.',{exact:true}).waitFor();assert.equal(await page.evaluate(()=>[...window.__mock.docs.values()].some(d=>d.title==='إعلان جديد معزول')),false);pass('failed atomic save leaves no active ad');await page.evaluate(()=>window.__mock.failTransaction=false);
 await page.getByRole('button',{name:'حفظ الإعلان',exact:true}).click();await page.waitForFunction(()=>[...window.__mock.docs].some(([p,d])=>p.startsWith('commercialAds/')&&d.title==='إعلان جديد معزول'));pass('owner add ad');assert.equal(await page.evaluate(()=>{const [key,ad]=[...window.__mock.docs].find(([p,d])=>p.startsWith('commercialAds/')&&d.title==='إعلان جديد معزول');return ad.imageUrl===''&&!JSON.stringify(ad).includes('base64')&&window.__mock.docs.get('commercialAdImages/'+key.split('/')[1])?.imageData.startsWith('data:image/jpeg;base64,');}),true);pass('metadata and separate image saved together');
