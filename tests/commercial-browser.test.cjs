...
(async () => {
 ...
 const collector=await page.evaluate(async()=>{const {installCommercialPublic}=await import('./commercial-public.js');const writes=[];const installed=await installCommercialPublic({...window.__mock.api,db:{},getDoc:async()=>({data:()=>({enabled:true})}),setDoc:async(ref,data)=>writes.push(data)});installed.recorder.record('views','isolated');installed.recorder.record('views','isolated');installed.recorder.record('clicks','isolated');await installed.recorder.flush();installed.dispose();return writes;});assert.ok(collector.length);assert.equal(collector.at(-1).views.filter(id=>id==='isolated').length,1);assert.equal(collector.at(-1).clicks.filter(id=>id==='isolated').length,1);assert.deepEqual(Object.keys(collector.at(-1)).sort(),['clicks','pageViews','pages','startedAt','updatedAt','views']);pass('enabled public collector dedupes without personal fields');
