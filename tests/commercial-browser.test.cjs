...
(async () => {
 ...
 await page.addInitScript(() => {
   ...
   const setUser = async user => { if(user)user.getIdToken=async()=>'fixture-token'; auth.currentUser = user; await Promise.all(observers.map(fn => fn(user))); };
   const mock = {
 ...
 await page.route('**/*', async route => {
   ...
   if(url.pathname==='/trusted-backend-client.js')return route.fulfill({contentType:'text/javascript',body:fs.readFileSync(path.join(root,'trusted-backend-client.js'),'utf8').replace('TRUSTED_BACKEND_ENABLED = false','TRUSTED_BACKEND_ENABLED = true').replace("signingEndpoint:''","signingEndpoint:'https://auth.test/sign-image'")});
   if(url.pathname==='/sign-image')return route.fulfill({json:{cloudName:'demo',apiKey:'fixture-key',signature:'fixture-signature',resourceType:'image',expiresAt:Math.floor(Date.now()/1000)+3600,params:{timestamp:Math.floor(Date.now()/1000),folder:'souq-alhalal/commercial-ads',public_id:'12345678-1234-4123-8123-123456789abc',overwrite:false,upload_preset:'commercial_ads_signed_v1',transformation:'c_limit,w_1600,h_1600',allowed_formats:'jpg,jpeg,png,webp'}}});
   if(url.hostname==='api.cloudinary.com')return route.fulfill({json:{resource_type:'image',public_id:'souq-alhalal/commercial-ads/12345678-1234-4123-8123-123456789abc',format:'jpg',bytes:1024,width:1600,height:800,secure_url:'https://res.cloudinary.com/demo/image/upload/souq-alhalal/commercial-ads/12345678-1234-4123-8123-123456789abc.jpg'}});
 ...
 const photo=await page.evaluate(()=>{const c=document.createElement('canvas');c.width=1600;c.height=800;c.getContext('2d').fillRect(0,0,1600,800);return c.toDataURL('image/png').split(',')[1];});await page.locator('.commercial-image-picker input[type=file]').setInputFiles({name:'ad.png',mimeType:'image/png',buffer:Buffer.from(photo,'base64')});await page.getByRole('button',{name:'رفع الصورة',exact:true}).click();await page.waitForFunction(()=>!document.querySelector('.commercial-ad-form button[type=submit]').disabled);await page.locator('[name=status]').selectOption('active');await page.getByRole('button',{name:'حفظ الإعلان',exact:true}).focus();await page.keyboard.press('Tab');assert.equal(await page.evaluate(()=>document.activeElement.textContent),'إغلاق');await page.keyboard.press('Shift+Tab');assert.equal(await page.evaluate(()=>document.activeElement.textContent),'حفظ الإعلان');pass('dialog keyboard focus contained');await page.screenshot({path:path.join(shots,'add-ad.png'),fullPage:true});for(const width of [768,390]){await page.setViewportSize({width,height:900});await page.locator('.commercial-image-picker').scrollIntoViewIfNeeded();assert.equal(await page.locator('.commercial-image-picker').evaluate(el=>el.scrollWidth<=el.clientWidth+1),true);await page.screenshot({path:path.join(shots,'add-ad-'+width+'.png')});pass('upload form responsive '+width);}await page.setViewportSize({width:1440,height:900});await page.getByRole('button',{name:'حفظ الإعلان',exact:true}).click();await page.waitForFunction(()=>[...window.__mock.docs].some(([p,d])=>p.startsWith('commercialAds/')&&d.title==='إعلان جديد معزول'));pass('owner add ad');
