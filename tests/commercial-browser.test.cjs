...
(async () => {
 ...
 await page.getByRole('button',{name:'إضافة إعلان جديد',exact:true}).click();await page.locator('dialog[open]').waitFor();assert.equal(await page.locator('[name=heroOrder] option').count(),10);pass('Hero order selector exposes ten occupancy labels');for(const [key,value]of Object.entries({title:'تعارض معزول',advertiserName:'معلن الاختبار',imageUrl:'https://res.cloudinary.com/demo/image/upload/sample.jpg',targetUrl:'https://example.test/',startAt:'2026-01-01T10:00',endAt:'2028-01-01T10:00'}))await page.locator('[name='+key+']').fill(value);await page.locator('[name=heroOrder]').selectOption('1');await page.getByRole('button',{name:'حفظ الإعلان',exact:true}).click();await page.getByText('هذا الترتيب مستخدم خلال الفترة المحددة. اختر مكانًا متاحًا.',{exact:true}).waitFor();assert.equal(await page.evaluate(()=>[...window.__mock.docs.values()].some(a=>a.title==='تعارض معزول')),false);pass('occupied Hero slot blocked without write');await page.getByRole('button',{name:'إغلاق',exact:true}).click();
 ...
 await page.evaluate(()=>window.openAdminSection('visitorAnalytics'));await page.getByRole('table').waitFor();assert.match(await page.locator('.commercial-admin').innerText(),/جمع الإحصاءات غير مفعّل حاليًا/);await page.screenshot({path:path.join(shots,'analytics.png'),fullPage:true});pass('disabled analytics explicitly identified without invented counts');
