...
const chooseFiles=async count=>page.evaluate(({data,count})=>{const bytes=Uint8Array.from(atob(data),c=>c.charCodeAt(0)),transfer=new DataTransfer();for(let i=0;i<count;i++)transfer.items.add(new File([bytes],'photo'+i+'.png',{type:'image/png'}));const input=document.getElementById('animalImages');input.files=transfer.files;input.dispatchEvent(new Event('change',{bubbles:true}));},{data:image,count});
...
assert.equal(await page.locator('#phoneNumber').count(),0);assert.equal(await page.locator('.v2-social button').count(),3);
for(const provider of ['Google','Facebook','X']){await page.evaluate(()=>window.__mock.socialError='auth/operation-not-allowed');await page.getByRole('button',{name:'المتابعة باستخدام '+provider,exact:true}).click();await page.getByText('تسجيل الدخول عبر '+provider+' غير متاح حاليًا',{exact:true}).waitFor();}pass('Google Facebook X unavailable gracefully; no public phone login');
await page.evaluate(()=>window.__mock.socialError='auth/account-exists-with-different-credential');await page.getByRole('button',{name:'المتابعة باستخدام Facebook',exact:true}).click();assert.match(await page.locator('#emailAuthStatus').innerText(),/الطريقة الأصلية/);assert.equal(await page.evaluate(()=>window.__mock.api.getAuth().currentUser),null);pass('provider collision preserves account and does not auto-link');
await page.evaluate(()=>{window.__mock.socialError=null;window.__mock.popupBlocked=true;});await page.getByRole('button',{name:'المتابعة باستخدام X',exact:true}).click();assert.equal(await page.evaluate(()=>window.__mock.calls.filter(c=>c.kind==='redirect').length),1);await page.evaluate(()=>{window.__mock.popupBlocked=false;sessionStorage.removeItem('souqSocialRedirect');});pass('blocked popup falls back to Firebase redirect');
await page.evaluate(()=>Object.defineProperty(navigator,'userAgent',{configurable:true,value:'Mozilla/5.0 (Linux; Android 14; wv)'}));await page.getByRole('button',{name:'المتابعة باستخدام Google',exact:true}).click();assert.match(await page.locator('#emailAuthStatus').innerText(),/متصفح الهاتف/);await page.evaluate(()=>delete navigator.userAgent);pass('WebView receives email/browser guidance without unsupported OAuth');
await page.evaluate(()=>window.__mock.socialError=null);await page.getByRole('button',{name:'المتابعة باستخدام Google',exact:true}).click();await page.waitForFunction(()=>!!window.__mock.api.getAuth().currentUser);assert.equal(await page.evaluate(()=>window.__mock.api.getAuth().currentUser.uid),'owner');assert.equal(await page.evaluate(()=>[...window.__mock.docs.keys()].filter(k=>k==='users/owner').length),1);pass('social login reuses UID and preserves existing profile');
