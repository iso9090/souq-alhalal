...
await test('index exactly matches actual equality and order fields',()=>{const actual=JSON.parse(fs.readFileSync(new URL('../firestore.indexes.json',import.meta.url),'utf8'));assert.deepEqual({...actual,indexes:actual.indexes.filter(i=>i.collectionGroup!=='analyticsEvents')},{indexes:[{collectionGroup:'commercialAds',queryScope:'COLLECTION',fields:[{fieldPath:'status',order:'ASCENDING'},{fieldPath:'endAt',order:'ASCENDING'},{fieldPath:'startAt',order:'ASCENDING'}]}],fieldOverrides:[]});});
