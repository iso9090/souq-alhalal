...
for(const f of ['legacy-index.html','_legacy/legacy-index.html','about.html','delete-account.html','payment-demo.html','app.js','admin-dashboard.js','payment-demo.js','melkak/fixtures.js','melkak/datasource.js','melkak/demo-auth.js','tests/melkak-review-smoke.test.cjs'])assert.equal(published.includes(f),false,f+' must not publish');
for(const f of ['index.html','melkak/entry.js','melkak/environment.js','melkak/runtime-config.js','melkak/production-bootstrap.js','melkak/style.css','melkak/app.js','melkak/auth-adapter.js','melkak/config.js','melkak/model.js','admin-permissions.js'])assert.ok(published.includes(f),f+' must publish');
for(const f of JSON.parse(fs.readFileSync('scripts/production-publish-manifest.json','utf8')).files)assert.ok(published.includes(f),f+' production dependency must publish through Pages');
