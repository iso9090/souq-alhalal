...
(async () => {
 ...
 assert.equal(await page.getByRole('button',{name:'إضافة مساعد',exact:true}).isVisible(),true);pass('Super Admin access and assistant list');
 for(const width of [1280,1366,360]){
  await page.setViewportSize({width,height:width===360?800:width===1366?768:900});await open();
  ...
  await page.getByRole('button',{name:'تفاصيل المساعد',exact:true}).click();await page.getByRole('button',{name:'تعديل الصلاحيات',exact:true}).waitFor();
  ...
  await page.getByRole('button',{name:'تعديل الصلاحيات',exact:true}).click();await page.locator('.admin-permissions-form').waitFor();
 ...
 await page.getByRole('button',{name:'إضافة مساعد',exact:true}).click();await page.locator('input[name=contact]').fill('unknown@example.test');await page.getByRole('button',{name:'بحث عن مستخدم',exact:true}).click();
 ...
 await page.getByRole('button',{name:'سارة مستخدمة الاختبار',exact:true}).click();await page.locator('.admin-permissions-form').waitFor();await shot('add-assistant-1280');
 ...
 await page.waitForFunction(()=>window.__mock.docs.get('adminAccess/candidate')?.role==='admin_assistant');await page.getByRole('button',{name:'تعديل الصلاحيات',exact:true}).waitFor();
 ...
 await page.getByRole('button',{name:'تعديل الصلاحيات',exact:true}).click();await page.locator('input[value=reports_view]').check();await page.locator('textarea[name=reason]').fill('توسيع العرض بالمحاكاة');await page.getByRole('button',{name:'حفظ صلاحيات المساعد',exact:true}).click();
 await page.waitForFunction(()=>window.__mock.docs.get('adminAccess/candidate').permissions.includes('reports_view'));await page.getByRole('button',{name:'إيقاف المساعد',exact:true}).waitFor();pass('edit assistant permissions');
 await page.getByRole('button',{name:'إيقاف المساعد',exact:true}).click();await page.getByRole('button',{name:'إعادة تفعيل المساعد',exact:true}).waitFor();await shot('suspended-assistant-1280');
 ...
 await page.getByRole('button',{name:'إعادة تفعيل المساعد',exact:true}).click();await page.getByRole('button',{name:'إيقاف المساعد',exact:true}).waitFor();pass('reactivate assistant');
 await page.getByRole('button',{name:'إزالة صلاحية مساعد مدير',exact:true}).click();await page.waitForFunction(()=>window.__mock.docs.get('adminAccess/candidate').role==='removed');
