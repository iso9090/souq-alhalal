...
for(const [id,ap] of [['paused',{status:'paused',approvedBy:'',approvedAt:null}],['pending',{status:'pending',approvedBy:'',approvedAt:null}],['expired',{startAt:Timestamp.fromMillis(Date.now()-172800000),endAt:Timestamp.fromMillis(Date.now()-86400000)}],['future',{startAt:Timestamp.fromMillis(Date.now()+86400000),endAt:Timestamp.fromMillis(Date.now()+172800000)}]]){await assertSucceeds(save(owner,id,{},ap));await test(id+' public image denied',()=>assertFails(getDoc(doc(guest,'commercialAdImages',id))));await test(id+' owner can read',()=>assertSucceeds(getDoc(doc(owner,'commercialAdImages',id))));}
