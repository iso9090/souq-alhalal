...
await page.evaluate(async()=>{window.__mock.delay=0;await window.logoutUser();await window.showMyBids()});await page.locator('#phoneNumber').waitFor();assert.equal(await page.locator('.ux-bid-card').count(),0);pass('guest redirected to login, private cards removed');
