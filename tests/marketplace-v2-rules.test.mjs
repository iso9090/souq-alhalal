...
await test('new duplicate image references rejected on server',()=>assertFails(setDoc(doc(seller,'animals','duplicate'),listing(['a','a']))));
await test('replacement with duplicate reference rejected on server',()=>assertFails(updateDoc(doc(seller,'animals','three'),{images:['a','a','c']})));
await env.withSecurityRulesDisabled(c=>setDoc(doc(c.firestore(),'animals','legacy-duplicate'),{sellerId:'seller',saleType:'direct',status:'active',images:['a','a']}));
await test('legacy duplicate references survive unrelated edit',()=>assertSucceeds(updateDoc(doc(seller,'animals','legacy-duplicate'),{breed:'قديمة'})));
await test('legacy duplicate reference can be removed',()=>assertSucceeds(updateDoc(doc(seller,'animals','legacy-duplicate'),{images:['a']})));
...
await test('legacy edit must reach at most three, not four',()=>assertFails(updateDoc(doc(seller,'animals','old'),{images:['a','b','c','d']})));
