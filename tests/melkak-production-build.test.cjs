...
const config=fs.readFileSync(path.join(out,'melkak/runtime-config.js'),'utf8');assert.match(config,/writesEnabled[^a-z]+true/);assert.match(config,/releaseId:'melkak-production-v1'/);assert.match(config,/includeLegacy:false/);assert.match(config,/modernCommercial:true/);assert.doesNotMatch(config,/unconfigured/);
