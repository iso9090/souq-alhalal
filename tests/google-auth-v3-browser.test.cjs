...
(async()=>{
 ...
 try{
  ...
  assert.equal(created.displayName,'مستخدم Google');
  assert.deepEqual(Object.keys(created).sort(),['accountType','createdAt','displayName','lastLoginAt','status','uid']);
  assert.equal(created.accountType,'buyer');assert.equal(created.status,'active');assert.ok(created.createdAt);assert.ok(created.lastLoginAt);
  ...
  assert.deepEqual(keys,[]);pass('existing Google profile login performs no writes');
