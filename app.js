...
window.openListingDetails = async function (animalId, auctionId = '', back = 'market') {
  ...
  try {
    ...
    showModal(`<section class="ux-detail" data-animal="${escapeHtml(animalId)}" data-auction="${escapeHtml(auction?.id || '')}" data-back="${escapeHtml(back)}">${backHtml}
      ...
      <div class="listing-actions" dir="rtl">
      ${auction ? `<p>سعر البداية: ${money(auction.startPrice,effectiveCountry(auction))}</p><p>أقل زيادة: ${money(auction.minIncrement,effectiveCountry(auction))}</p><p>موعد الانتهاء: ${formatDate(auction.endTime)}</p><p ${!expired ? `data-auction-end="${timestampToMillis(auction.endTime)}" data-auction-id="${escapeHtml(auction.id)}"` : ''}>${expired ? 'انتهى المزاد' : getCountdownText(auction.endTime)}</p>${auctionActionHtml(auction,expired,owner).replace('id="bid-button-', 'id="detail-bid-button-')}` : animal.saleType === 'auction' ? '<p>تفاصيل المزاد غير متاحة حاليًا.</p>' : !owner && (!animal.status || animal.status==='active') ? `<button type="button" class="listing-action listing-action--purchase" onclick="requestPurchase(${inlineArgument(animalId)})"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3 3h2l3 12h11l2-8H6M9 20h.01M18 20h.01"/></svg><span>طلب شراء</span></button>` : `<p>${animal.status==='sold' ? 'تم البيع' : 'هذا إعلانك'}</p>`}
      ${!owner && animal.saleType==='direct' ? `<div class="listing-actions-secondary"><button type="button" class="listing-action listing-action--message" onclick="openDirectConversation(${inlineArgument(animalId)})"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 4h16v12H9l-5 4V4Z"/></svg><span>مراسلة البائع</span></button></div><p class="listing-offer-note">تقديم عرض السعر متاح داخل المحادثة.</p>` : ''}
      <button type="button" class="listing-action listing-action--report" onclick="submitModerationReport('animal', ${inlineArgument(animalId)})"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 21V3m0 1h13l-3 5 3 5H5"/></svg><span>الإبلاغ عن الإعلان</span></button>
      <button type="button" class="listing-action listing-action--report-seller" onclick="submitModerationReport('user', ${inlineArgument(animal.sellerId)})">الإبلاغ عن البائع</button>
      </div>
