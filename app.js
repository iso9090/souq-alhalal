...
function getCountdownText(endTime) {
  ...
  if (days > 0) {
    return "متبقي " + days + " يوم و " + hh + ":" + mm + ":" + ss;
  }
  ...
  showModal(`
    <div style="direction:rtl;color:white;padding:12px;">
      ...
      <select
        ...
        style="width:100%;padding:14px;margin:8px 0 18px;"
      >
        <option value="buyer" ${accountType === "buyer" ? "selected" : ""}>
          مشتري
        </option>

        <option value="seller" ${accountType === "seller" ? "selected" : ""}>
          بائع
        </option>

        <option value="both" ${accountType === "both" ? "selected" : ""}>
          بائع ومشتري
        </option>
      </select>
...
window.showMyListings = async function () {
  ...
  try {
    ...
    const cards = myAnimals.map(animal => {
      ...
      const auction =
        auctionsByAnimal[animal.id] || null;
      ...
      if (animal.saleType === "auction") {
        ...
        if (auction) {
          ...
          const endMillis =
            timestampToMillis(auction.endTime);
      ...
      <div style="direction:rtl;color:white;padding:12px;">
        ...
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:7px;margin:18px 0;">
          ...
          <div style="background:#123c2c;border-radius:10px;padding:10px 5px;text-align:center;">
            ...
            <div style="color:#68e6b0;font-size:22px;font-weight:bold;">
              ${activeCount}
            </div>
          ...
          <div style="background:#421d1d;border-radius:10px;padding:10px 5px;text-align:center;">
            ...
            <div style="color:#ff8d8d;font-size:22px;font-weight:bold;">
              ${closedCount}
            </div>
  ...
  try {
    ...
    showModal(`
      <div style="direction:rtl;color:white;padding:12px;">
        ...
        <h2 style="color:#68e6b0;text-align:center;">
          📋 طلباتي
        </h2>
...
window.updatePurchaseRequest = async function (
  requestId,
  newStatus
) {
...
window.openLogin = async function () {
  ...
  showModal(`
    <div style="direction:rtl;color:white;padding:10px;">
      ...
      <h2 style="text-align:center;color:#68e6b0;">
        تسجيل الدخول
      </h2>
...
function showCodeScreen(phone) {
  ...
  showModal(`
    <div style="direction:rtl;color:white;padding:10px;">
      ...
      <p
        dir="ltr"
        style="text-align:center;color:#68e6b0;font-weight:bold;"
      >
        ${escapeHtml(phone)}
      </p>
...
function createFirebaseArea() {
  ...
  area.innerHTML = `
    <div style="max-width:1100px;margin:35px auto;padding:20px;direction:rtl;">
      ...
      <h2 style="text-align:center;color:#68e6b0;margin-bottom:7px;">
        🐪 سوق الحلال
      </h2>
      ...
      <h2 style="margin-top:40px;color:#68e6b0;">
        🛒 البيع المباشر
      </h2>
      ...
      <h2 style="margin-top:50px;color:#68e6b0;">
        🔨 المزاد الإلكتروني
      </h2>
...
window.manageListing = async function (
  animalId
) {
...
window.finalizeAuction = async function (
  auctionId,
  decision
) {
...
window.saveListingEdits = async function (
  animalId
) {
...
window.removeAnimalImage = async function (
  animalId,
  imageIndex
) {
...
window.replaceAnimalImages = async function (
  animalId
) {
...
window.markListingSold = async function (
  animalId
) {
...
window.requestPurchase = async function (
  animalId
) {
