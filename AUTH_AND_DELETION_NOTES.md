...
Support mailbox: soqalhalal9@gmail.com. Developer/store identity: WAL DIGITAL. In-app requests are stored in Firestore; they do NOT automatically send email or notify staff. Staff must monitor the new “طلبات حذف الحسابات” section in the trusted-claim admin panel (or Firebase Console), alongside the support mailbox. Existing service controls are unchanged. No automatic deletion processor exists. The section uses UID, safely resolved display name, dates and status only; pending → in_review → completed uses a transaction with audit fields and a serious confirmation before completion.
...
A newly created deletion request closes its modal, confirms submission and signs out via the existing signOut flow. Existing requests are handled without another write or logout. Auth restoration checks the request; Account refreshes the check and shows pending/in_review warnings with a disabled deletion action. A completed request reports the recorded admin status and support contact; if Auth still exists, login remains possible, which is an operational limitation. No client Auth deletion or cascade is implemented. Public deletion/privacy text matches this behavior. Terms/about required no change. Firestore rules are unchanged and were not redeployed in this follow-up.

Follow-up validation: Firestore emulator 51/51 (including explicit user-completion and forged timestamp denials), UI/source 85/85, auth/deletion browser 22/22 and original browser suite PASS. Both 360/1280 widths pass without horizontal overflow. All test writes are mocked or emulator-only. Java/Netty deprecated Unsafe warnings and intentional PERMISSION_DENIED assertions are test-runtime output, not deployment failures. app.js cache 59 → 60; launch.css remains version 2 unchanged. Android source reviewed only; no rebuild or file modifications.
